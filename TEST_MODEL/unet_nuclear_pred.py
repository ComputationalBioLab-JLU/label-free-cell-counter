...
class Unet_model():
    ...
    def __call__(self,img_path,save_path):
        ...
        pic_dict,x_side_number,y_side_number=self.slice_pic(img_array,572,572,self.unet_model)
        pred_array=self.joint_pic(pic_dict,x_side_number,y_side_number)
    ...
    def slice_pic(self,pic_array,slice_size,stride,unet_model):
        ...
        return([output_dict,x_side_number,y_side_number])
    def joint_pic(self,pic_dict,x_number,y_number):
        pic_lines=[]
        for x_index in range(x_number):
            init_x_array=pic_dict['{}-0'.format(x_index)]
            for y_index in range(1,y_number):
...
def test_output(data_path,data_suffix,copy_suffix_list,save_path):
    ...
    model=Unet_model('/state/heyang/zheda_cell_project/Old_data_retrain/code/Unet_part/unet_model/95_checkpoint.pth')
    ...
    for data_path in (data_list):
        ...
        save_output_path='{}/Unet_{}'.format(save_path,os.path.basename(data_path)[2:])
